...
class TestImpalaShell(ImpalaTestSuite):
  ...
  def test_output_format(self, vector):
    ...
    args = ['-q', 'select 1 as col_first, 2 as col_second, 3 as col_third', '--quiet']
    ...
    assert "************************************** " \
      "1.row **************************************" == result_rows[0]
    assert " col_first: 1" == result_rows[1]
    assert "col_second: 2" == result_rows[2]
    assert " col_third: 3" == result_rows[3]
